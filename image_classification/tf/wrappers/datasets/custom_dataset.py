...
from image_classification.tf.src.datasets.utils import preprocess_data, prepare_kwargs_for_dataloader, download_dataset
from image_classification.tf.src.datasets.custom_dataset import load_custom_dataset


__all__ = ['get_custom_dataset']

@DATASET_WRAPPER_REGISTRY.register(framework='tf', dataset_name='custom_dataset', use_case="image_classification")
def get_custom_dataset(cfg):

    """
    Loads the images from the custom dataset, pre-process them and return training, validation, and test tf.data.Datasets.
    ...
    # Creates datasets
    dataloaders = load_custom_dataset(training_path=args['training_path'],
