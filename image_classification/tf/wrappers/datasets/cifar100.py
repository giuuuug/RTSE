...
from image_classification.tf.src.datasets.cifar100 import load_cifar100
from image_classification.tf.src.datasets import prepare_kwargs_for_dataloader


__all__ = ['get_cifar100']

@DATASET_WRAPPER_REGISTRY.register(framework='tf', dataset_name='cifar100', use_case="image_classification")
def get_cifar100(cfg): 
    
    """
    Loads the images from the cifar100 dataset, pre-process them and return training, validation, and test tf.data.Datasets.
    ...
    # Creates datasets
    dataloaders = load_cifar100(training_path=args['training_path'],
