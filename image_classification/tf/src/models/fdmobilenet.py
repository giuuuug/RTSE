...
def _create_fd_mobilenet(input_shape: List[int] = [224, 224, 3], alpha: float = 1.0, depth_multiplier: int = 1,
                        ...
                        classes: int = 101, **kwargs) -> keras.Model:
    """
    ...
    Returns:
        model (keras.Model): A Keras model for fine-grained classification.
    ...
    model = keras.Model(img_input, x, name='fdmobilenet_%0.2f' % alpha)
    return model
...
def get_fdmobilenet(input_shape: Tuple[int, int, int] = None, num_classes: int = None, alpha: float = None,
    ...
    if pretrained:
      print("WARNING: No pretrained weights are found for 'fdmobilenet' model. Random weights are used instead.")
