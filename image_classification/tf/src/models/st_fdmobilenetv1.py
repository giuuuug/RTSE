...
def _create_st_fdmobilenetv1(input_shape: list[int] = [224, 224, 3], alpha: list[float] = [1.0],
                                    depth_multiplier: int = 1, dropout: float = 1e-3,
                                    input_tensor: Optional[tf.Tensor] = None, classes: int = 101) -> keras.Model:
    ...
    model = keras.Model(img_input, x, name='st_fdmobilenetv1')

    return model


def get_st_fdmobilenetv1(input_shape: Tuple[int, int, int] = None, num_classes: int = None,
    ...
    if pretrained:
      print("WARNING: No pretrained weights are found for 'st_fdmobilenetv1' model. Random weights are used instead.")
    ...
    # Create model
    model = _create_st_fdmobilenetv1(input_shape=input_shape, alpha=alpha_list, depth_multiplier=1,
