...
def main(args):
    ...
    audio_clips_list = []
    p808_model_path = os.path.join(ENV, 'DNSMOS', 'model_v8.onnx')

    if args.personalized_MOS:
        primary_model_path = os.path.join(ENV, 'pDNSMOS', 'sig_bak_ovr.onnx')
    else:
        primary_model_path = os.path.join(ENV, 'DNSMOS', 'sig_bak_ovr.onnx')
