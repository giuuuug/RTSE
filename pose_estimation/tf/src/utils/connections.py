...
"""
Below are all the default connections for FaceMesh and pose/hand skeleton drawing.
You typically do not need to modify these.

If you want to customize / add your own skeleton (e.g. add/remove segments or change colors),
scroll to the bottom of this file and edit/extend the dictionaries :
 - `stm32_to_opencv_colors_dict` to define the BGR values of colors used in `skeleton_connections_dict`
 - `skeleton_connections_dict` is organized in three levels:
      1. First level: dict of classes -> "head", "hand", "person", ...
      2. Second level: dict of number of landmarks -> 17, 21, 468, ...
      3. Third level: list of triplets -> [start_index, end_index, "UTIL_LCD_COLOR_*"]
        where:
          * start_index and end_index are landmark indices to connect.
          * "UTIL_LCD_COLOR_*" is a color name that can be mapped to a BGR
            value using `stm32_to_opencv_colors_dict`.
