...
class TFLiteQuantizedModelPredictor:
    """
    A class to handle predictions using a TFLite model. This class includes methods for:
    ...
    def __init__(self, cfg, model, dataloaders):
        ...
        Args:
            ...
            model: The trained TFLite model to use for predictions.
    ...
    def _get_prediction(self, img, target, ai_runner_interpreter):
        ...
        input_shape_model = input_details['shape']  # e.g., [1, 224, 224, 3]
        height_model, width_model = input_shape_model[1], input_shape_model[2]
        if input_details['dtype'] == np.uint8:
            img = (img - self.cfg.preprocessing.rescaling.offset) / self.cfg.preprocessing.rescaling.scale
            img = np.clip(np.round(img), np.iinfo(input_details['dtype']).min, np.iinfo(input_details['dtype']).max)
        ...
        if target == 'host':
            ...
            data        = ai_interp_input_quant(ai_runner_interpreter,img,'.tflite')
        ...
        for idx,pr in enumerate(predictions):
            if outputs_details[idx]['dtype'] in [np.uint8,np.int8]:
                predictions[idx] = ((predictions[idx] - outputs_details[idx]['quantization'][1]) * outputs_details[idx]['quantization'][0]).astype('float32')
        ...
        if self.model_type!='hand_spe':
            predictions = predictions[0]

        if self.cfg.evaluation:
            if self.cfg.evaluation.output_chpos=="chfirst" and target != 'host':
        ...
        if self.model_type=='heatmaps_spe':
            ...
            print('No post-processing found for the TFLite model type : '+self.model_type)
