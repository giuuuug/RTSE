...
from pose_estimation.tf.src.postprocessing import heatmaps_spe_postprocess
...
def single_pose_oks(y_true:tf.Tensor, y_pred:tf.Tensor):
    ...
    def f3(): return tf.ones(nb_kpts,tf.float32)/tf.cast(nb_kpts,tf.float32)
...
def single_pose_heatmaps_oks(y_true:tf.Tensor, y_pred:tf.Tensor, image_size:tf.Tensor, network_stride:int):
    '''
    ...
    ps     = tf.cast(image_size / network_stride,tf.int64)

    y_pred = heatmaps_spe_postprocess(y_pred,pred_size=ps)
