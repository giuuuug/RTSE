...
def keypoints_normalized_to_abs_coords(
    keypoints: tf.Tensor,
    image_size: tuple,
    clip_points: bool = False
):
    """
    Converts normalized keypoints to absolute pixel coordinates.

    Args:
        keypoints: Tensor of shape [batch, num_points, 2] or [batch, num_points*2]
        image_size: Tuple (width, height)
        clip_points: If True, clips coordinates to image bounds

    Returns:
        Tensor of shape [batch, num_points, 2] with absolute coordinates
    """
    image_size = tf.cast(image_size, tf.float32)
    width, height = image_size[0], image_size[1]

    # If input is [batch, num_points*2], reshape to [batch, num_points, 2]
    if keypoints.shape.rank == 3 and keypoints.shape[-1] == 2:
        kp = keypoints
    else:
        num_points = keypoints.shape[-1] // 2
        kp = tf.reshape(keypoints, [-1, num_points, 2])

    x = tf.round(kp[..., 0] * width)
    y = tf.round(kp[..., 1] * height)

    if clip_points:
        x = tf.clip_by_value(x, 0, width - 1)
        y = tf.clip_by_value(y, 0, height - 1)

    abs_keypoints = tf.stack([x, y], axis=-1)  # [batch, num_points, 2]
    abs_keypoints = tf.expand_dims(abs_keypoints, axis=0)
...
def plot_bounding_boxes(ax, boxes,key_points, classes=None, scores=None, class_names=None, colors=None):
    ...
    for i in range(num_boxes):
        ...
        # key_points[i] shape: (num_keypoints, 2)
        for kp in key_points[i]:
            ax.scatter(kp[0], kp[1], c='lime', s=20, marker='o', edgecolors='black', linewidths=0.5, zorder=3)
