...
from face_detection.tf.src.postprocessing import nms_box_keypoints_filtering
...
class ObjectDetectionMetricsData:
    ...
    def __init__(self, num_labels, num_boxes, num_classes, num_detections, dataset_size, batch_size, name=None, **kwargs):
        ...
        self.batch_keypoints = tf.Variable(tf.zeros([batch_size, num_detections, 12]), trainable=False, dtype=tf.float32)
    ...
    def add_data(self, gt_labels, boxes, scores, keypoints):
        """
        ...
        self.batch_keypoints.scatter_nd_update(indexes, keypoints)
    ...
    def update_batch_index(self, batch, nms_score_threshold, nms_iou_threshold, image_size):
        ...
        # NMS the predictions
        boxes, scores, classes, nmsed_keypoints = nms_box_keypoints_filtering(
                            self.batch_boxes[:self.current_batch_size], self.batch_scores[:self.current_batch_size],self.batch_keypoints[:self.current_batch_size],
