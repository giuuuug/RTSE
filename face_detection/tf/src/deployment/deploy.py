...
from common.deployment import stm32ai_deploy_stm32n6, stm32ai_deploy
from common.benchmarking import cloud_connect
from face_detection.tf.src.utils import gen_h_user_file_n6_blazeface, gen_h_user_file_n6_yunet
...
def deploy(cfg: DictConfig = None, model_path_to_deploy: Optional[str] = None) -> None:
    ...
    # if yunet use onnx header generator
    if cfg.model.model_type == "yunet" and model_path.endswith(".onnx"):
        tpp, quantized_model_path = gen_h_user_file_n6_yunet(config=cfg, quantized_model_path=model_path)
    elif cfg.model.model_type == "facedetect_front" and model_path.endswith(".tflite"):
        tpp, quantized_model_path = gen_h_user_file_n6_blazeface(config=cfg, quantized_model_path=model_path)
    else:
        raise TypeError("Only `yunet` model in ONNX format and `facedetect_front` model in TFLite format are supported for now.")
    if tpp:
