# Face Detection STM32N6 Model Deployment

This tutorial demonstrates how to deploy a pre-trained Face Detection model built with quantized tflite or ONNX QDQ on an STM32N6 board using STEdgeAI.
...
The [STM32N6 application code](../../application_code/face_detection/STM32N6/README.md) submodule:
...
As an example, we will show how to deploy [yunetn_320_qdq_int8.onnx](https://github.com/STMicroelectronics/stm32ai-modelzoo/tree/master/face_detection/yunet/Public_pretrainedmodel_public_dataset/widerface/yunetn_320) using the necessary parameters provided in [deployment_n6_yunet_config.yaml](../config_file_examples/deployment_n6_yunet_config.yaml). To get this model, clone the [ModelZoo repo](https://github.com/STMicroelectronics/stm32ai-modelzoo/) in the same folder you cloned the [STM32 ModelZoo services repo](https://github.com/STMicroelectronics/stm32ai-modelzoo-services/).

To configure the deployment, edit [`../config_file_examples/deployment_n6_yunet_config.yaml`](../config_file_examples/deployment_n6_yunet_config.yaml).
...
The Face Detection models usually have a post processing to be applied to filter the model output and show final results on an image.
...
__4.__ Once [`deployment_n6_yunet_config.yaml`](../config_file_examples/deployment_n6_yunet_config.yaml) filled, launch:

```bash
python stm32ai_main.py --config-path ./config_file_examples/ --config-name deployment_n6_yunet_config.yaml
```
...
__Note__:
If you have an ONNX model that has not been quantized and you want to quantize it before deploying it, you can use the `chain_qd` tool to quantize and deploy the model sequentially. To do this, update the [chain_qd_n6_config.yaml](../config_file_examples/chain_qd_n6_config.yaml) file and then run the following command from the UC folder to build and flash the application on your board:
