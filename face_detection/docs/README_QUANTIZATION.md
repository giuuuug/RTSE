# Face Detection STM32 Model Quantization
...
All the sections of the YAML file must be set as described in the **[README.md](./README_OVERVIEW.md)** with the `operation_mode` set to quantization.
...
dataset:
  class_names: [person]
  quantization_path: /local/data/od_wider_face/test
...
- `quantizer` - *String*, "onnx_quantizer" or "TFlite_converter" which will convert model trained weights from float to integer values.
...
The quantized TFLite model or the onnx qdq can be found in the corresponding **experiments_outputs/** folder.
...
In case you want to evaluate your quantized model footprints, you can either launch the benchmark operation mode on the generated quantized model (please refer to the benchmarking **[README.md](./README_BENCHMARKING.md)** that describes in detail how to proceed) or you can use chained services like launching the [chain_eqeb](../config_file_examples/chain_eqeb_config.yaml) example with the command below:
