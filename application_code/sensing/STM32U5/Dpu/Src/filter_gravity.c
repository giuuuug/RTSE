...
/* Private variables ---------------------------------------------------------*/
static const float kGravityHighPassA[GRAVITY_HIGHPASS_N] =
{
  1.0, -3.868656635, 5.614526749, -3.622760773, 0.8768966198
};

static const float kGravityHighPassB[GRAVITY_HIGHPASS_N] =
{
  0.9364275932, -3.745710373, 5.618565559, -3.745710373, 0.9364275932
};

static const float kGravityHighPassInit[FILT_ORDER] =
{
  -0.936528250873, 2.809571532101, -2.809559172096, 0.936515859573
};

/* Private function prototypes -----------------------------------------------*/
static void iir_direct2_init(IIRFilterDirect2 * filter,
                      const float * a, const float * b, const float * z,
                      float first);

static float iir_direct2_filter(IIRFilterDirect2 * filter, float x);

static void dynamic_acceleration(float acc_x, float acc_y, float acc_z,
                          float * dyn_x, float * dyn_y, float * dyn_z);
/* Exported Functions --------------------------------------------------------*/
/**
* @brief  Remove gravity from acceleration raw data
* @param  Acceleration value (x/y/z)
* @retval Acceleration value filtered (x/y/z)
*/
acceleration_3D_t gravity_suppress_rotate(acceleration_3D_t * data)
{
...
acceleration_3D_t gravity_rotate(acceleration_3D_t * data)
{
  float dyn_x, dyn_y, dyn_z;
  dynamic_acceleration(data->AccX, data->AccY, data->AccZ, &dyn_x, &dyn_y, &dyn_z);
