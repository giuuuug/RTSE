# Re-Identification STM32 Model Quantization
...
This tutorial shows how to quantize a floating-point re-identification model with real data. As an example, we will demonstrate the workflow on a person re-identification dataset.
...
- `onnx_extra_options` - *Field*, only option supported for now is "CalibMovingAverage" that is a boolean and only applies to ONNX quantization. This field is optional.
