...
def gen_h_user_file_n6(config: DictConfig = None, quantized_model_path: str = None) -> None:
    ...
    if os.path.basename(quantized_model_path).endswith(".tflite"):
        ...
        model_output_shape = session.get_outputs()[0].shape
        output_shape = model_input_shape[-3:]
    ...
    with open(os.path.join(path, "app_config.h"), "wt") as f:
        ...
        f.write('\n');
        f.write('/* Re-id model Info*/\n');
        f.write('#define NN_REID_WIDTH                        ({})\n'.format(int(input_shape[1])));
        f.write('#define NN_REID_HEIGHT                       ({})\n'.format(int(input_shape[2])));
        f.write('\n');
        f.write('#define NN_REID_NB_FEATURES                  ({})\n'.format(int(output_shape[1])));
        f.write('\n');
        f.write('/* Application tuning */\n');
        f.write('#define APP_MAX_OBJECT_DETECT                ({})\n'.format(int(params.app.max_object_detect)));
        f.write('#define APP_MAX_OBJECT_TRACKING              ({})\n'.format(int(params.app.max_object_tracking)));
        f.write('#define APP_SCORE_THRESHOLD                  ({})\n'.format(float(params.app.score_threshold)));
        f.write('#define APP_LOST_TIME_IN_MS                  ({})\n'.format(int(params.app.lost_time_in_ms)));
        f.write('#define APP_START_TRACKING_CONF_THRESHOLD    ({})\n'.format(float(params.app.start_tracking_conf_threshold)));
        f.write('\n');
        f.write('/* Display */\n');
