...
from audio_event_detection.tf.src.models import add_head
...
def _check_parameters(embedding_size: int,
    ...
    print("Using Yamnet with patch_length: {}, n_mels: {}".format(patch_length, n_mels))
...
def _get_transfer_learning_model(num_classes: int = None,
    ...
    Inputs
    ------
    num_classes : int, number of neurons of the classification head
...
def get_yamnet(num_classes: int = None,
              embedding_size: int = None,
              fine_tune: bool = False,
              use_garbage_class: bool = False,
              dropout: float = 0.,
    ...
    pretrained : bool, must be True. Raises an appropriate error if False.
    ...
    if pretrained:
        yamnet = _get_transfer_learning_model(num_classes=num_classes,
        ...
        raise RuntimeError("Yamnet is only available with pretrained weights. \n \
                           Please set the pretrained arguments to True in \
