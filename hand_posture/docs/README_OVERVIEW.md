...
Remember that minimalistic yaml files are available [here](../config_file_examples/) to play with specific services, and that all pre-trained models in the [STM32 model zoo](https://github.com/STMicroelectronics/stm32ai-modelzoo/hand_posture) are provided with their configuration `.yaml` files used to generate them. These are very good starting points to start playing with!
...
3. [Visualize the services results](#3)
...
To use the services in the hand posture model zoo, you can utilize the model zoo [stm32ai_main.py](../stm32ai_main.py) along with the [user_config.yaml](../user_config.yaml) file as input. The YAML file specifies the service (operation_mode) and a set of configuration parameters such as the model (either from the model zoo or your own custom model), the dataset, the number of epochs, and the preprocessing parameters, among others.
...
**Note**: The log directory is an architecture generated by the ST datalog tool: STSW-IMG035_EVK (Gesture EVK).
This tool can be downloaded on [st.com](https://www.st.com/en/embedded-software/stsw-img035.html), a dedicated documentation is available for this tool and to know how to create your own ST multi-zone Time-of-Flight dataset for hand posture recognition.

</details>
<details open><summary><a href="#2"><b>2. Hand Posture tutorial</b></a></summary><a id="2"></a>

This tutorial demonstrates how to use the `training` services to train the model. We chose to use the `st_cnn2d_handposture` model and the [ST_VL53L8CX_handposture_dataset](../datasets/ST_VL53L8CX_handposture_dataset.zip) dataset as an example to demonstrate the workflow.
...
The `dataset_name` attribute mentioning the name of the dataset. For the moment the only supported dataset is ST_handposture_dataset collected either with STV53L5CX or VL53L8CX sensors.
...
The `model` section is used to specify the model to use for the selected operation mode. It is shown below.

```yaml
model:    # Use it if you want to use a model from the zoo, mutually exclusive with 'model.model_path'
  model_name: st_cnn2d_handposture # or custom_model
  input_shape: (8, 8, 2)
# model_path: <file-path>           # Use it if you want to use your own model, mutually exclusive with 'model.model_name'
```
</details></ul>
...
All the TensorFlow optimizers are supported. If you are not passing any argument to the optimizer, you may write it on one line. For example: `optimizer: Adam`.
...
The `callbacks` subsection is optional. All the TensorFlow callbacks are supported. Note that several callbacks are built-in and cannot be redefined, including ModelCheckpoint, TensorBoard, and CSVLogger.

The best model obtained at the end of the training is saved in the `./tf/src/experiments_outputs/<run-date-and-time>/saved_models` directory and is called `best_model.keras` (see section <a href="#4">Saved results of image classification</a>). Make sure not to use the `best_augmentation_model.keras` file as it includes the rescaling and data augmentation layers.
...
If you want to deploy the model after running the training, you can do so by referring to the [README](./README_DEPLOYMENT.md) and modifying the `deployment_config.yaml` file or by setting the `operation_mode` to `deployment` and modifying the `user_config.yaml` file as described below:

```yaml
model:
   model_path: <path-to-a-keras-model-file>     # Path to the model file to deploy
...
In the `model` section, users must provide the path to the Keras model file that user wants to deploy using the `model_path` attribute.

The `dataset` section requires users to provide the names of the classes using the `class_names` attribute.

The `tools` section includes information about the STEdge AI toolchain, such as the version, optimization level, and path to the `stedgeai.exe` file.
...
Every time you run an operation/service of the Model Zoo, an experiment directory is created that contains all the directories and files created during the run. The names of experiment directories are all unique as they are based on the launch date and time of the run.
...
By default, all the experiment directories are under the `<MODEL-ZOO-ROOT>/hand_posture/tf/src/experiments_outputs` directory and their names follow the "%Y_%m_%d_%H_%M_%S" pattern.
...
```
 ...
 saved_models                               logs             .hydra
      |                                      |                 |
      +--- best_augmented_model.keras      TensorBoard          Hydra
      +--- best_model.keras                   files             files
```
...
The models in the `best_augmented_model.keras` Keras files contain data augmentation layers. These model files are not intended to be used outside of the Model Zoo context.
...
To visualize the training curves that were logged by TensorBoard, navigate to the **./tf/src/experiments_outputs/\<date-and-time\>** directory and run the following command:
...
To view and examine the results of multiple trainings, you can navigate to the **./tf/src/experiments_outputs** directory and access the MLflow Webapp by running the following command:
...
model:    # Use it if you want to use a model from the zoo, mutually exclusive with 'model.model_path'
  model_name: st_cnn2d_handposture # custom
  input_shape: (8, 8, 2)
```
...
```yaml
attribute_1: st_cnn2d_handposture
attribute_2: "st_cnn2d_handposture"
attribute_3: 'st_cnn2d_handposture'
# are all equivalent
```
