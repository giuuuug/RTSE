...
from semantic_segmentation.tf.src.preprocessing import preprocess_image
...
def get_batch_data_and_masks(images, masks, nchw=True):
    """
    Converts images and masks to numpy arrays and transposes to NCHW if needed.
    Handles both 3D and 4D arrays.
    Returns (batch_data, batch_masks).
    """
    batch_data = tf.cast(images, dtype=tf.float32).numpy()
    batch_masks = tf.cast(masks, dtype=tf.float32).numpy()
