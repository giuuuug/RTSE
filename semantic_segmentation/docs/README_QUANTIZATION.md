...
This tutorial shows how to quantize a floating-point model with real data. As an example, we will demonstrate the workflow on the deeplabv3 model and COCO 2017 Pascal VOC 2012 segmentation dataset.

## <a id="">Table of contents</a>

<details open><summary><a href="#1"><b>1. Configure the yaml file</b></a></summary><a id="1"></a>

`operation_mode` should be set to quantization. 
...
dataset:
   ...
   quantization_path: ./datasets/person_COCO2017_VOC2012/JPEGImages
   quantization_masks_path: ./datasets/person_COCO2017_VOC2012/SegmentationClassAug
   quantization_files_path: ./datasets/person_COCO2017_VOC2012/ImageSets/Segmentation/quantization.txt
```
...
quantization:
  ...
  onnx_extra_options:     # Optional, defaults to None. Only used when using Onnx_quantizer
...
- `model_type` - *String*, specifies the model architecture to use for quantization. For semantic segmentation, the supported value is `deeplabv`.
...
- `onnx_extra_options` - *Field*, can contain `CalibMovingAverage` set as a Boolean, only used when "Onnx_quantizer" is used.
...
The 'onnx_extra_options' expects `CalibMovingAverage` set to True of False when using `Onnx_quantizer`. When it is set to True, the calibration process for quantizing the model will compute the moving average of the minimum and maximum values observed during the calibration phase. This helps in smoothing out fluctuations and providing a more stable calibration result. If set to False, the calibration will use the raw minimum and maximum values without averaging.
